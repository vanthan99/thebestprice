package dtu.thebestprice.services;

import dtu.thebestprice.payload.request.price.RetailerUpdatePriceRequest;
import org.springframework.http.ResponseEntity;

public interface PriceService {

    // retailer cập nhật giá
    ResponseEntity<Object> retailerUpdatePrice(long productRetailerId, RetailerUpdatePriceRequest priceRequest);

    // admin phê duyệt giá
    ResponseEntity<Object> adminApprovePrice(long productRetailerId);

    // admin cập nhật giá cho mọi sản phẩm
    ResponseEntity<Object> adminUpdatePrice(long productRetailerId, RetailerUpdatePriceRequest priceRequest);

    // admin lấy danh sách giá theo sản phẩm
    ResponseEntity<Object> adminGetPriceByProduct(long productId);
}
